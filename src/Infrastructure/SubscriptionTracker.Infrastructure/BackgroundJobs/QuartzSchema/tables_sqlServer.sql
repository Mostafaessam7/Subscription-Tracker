-- Official Quartz.NET SQL Server schema, sourced from
-- https://github.com/quartznet/quartznet/blob/main/database/tables/tables_sqlServer.sql
-- Adapted for QuartzSchemaInitializer (see that class): the "USE [enter_db_name_here]" header was
-- removed (the ADO.NET connection is already scoped to the right database) and @DropDb defaults to
-- 0 (non-destructive) since QuartzSchemaInitializer only ever runs this once, guarded by its own
-- "do the QRTZ_* tables already exist" check - don't flip @DropDb to 1 here, it would wipe scheduler
-- state on every deploy if the guard is ever bypassed.
