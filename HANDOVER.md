...
7. SignalR-cron-verification is next in this session's queue — check whether it's marked done above (§2/§6/§7) or still listed as open before assuming it's unstarted.

Test counts after items 4–6: see the running total at the top of §2 (kept current there, not duplicated here to avoid drift between the two numbers).
